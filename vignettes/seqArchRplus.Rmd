---
title: "seqArchRplus for downstream analysis of clusters of promoter sequence architectures"
author: 
...
vignette: >
  %\VignetteIndexEntry{seqArchRplus for downstream analysis of clusters of promoter sequence architectures}
...
`r Biocpkg("seqArchRplus")`, enables performing various steps in their 
downstream bioinformatic analyses and produce publication-ready plots 
...
- Produce BED track files of `r Biocpkg("seqArchR")` clusters for visualization in a genome
browser or IGV
Examples of most of these capabilities are provided in this vignette.


We hope that using `r Biocpkg("seqArchRplus")` (together with `r Biocpkg("seqArchR")`) will be useful in 
...
So far, we have tested `r Biocpkg("seqArchR")` and `r Biocpkg("seqArchRplus")` on promoters in drosophila 
(Schor et al. 2017 and modENCODE), zebrafish (Nepal et al. 2013), mice (Fantom),
humans (ENCODE), and also in plants (barley and maize).

We already have a plan for a feature in the future version of `r Biocpkg("seqArchRplus")`: 
...
## Latest stable version from Bioconductor
...
## Latest 'devel' version from GitHub
...
To begin using `r Biocpkg("seqArchRplus")`, you require a set of promoters sequences (as 
a DNAStringSet object) and their clustering information (as a simple list 
storing the sequence IDs belonging to each cluster). 
...
`r Biocpkg("seqArchRplus")` can use these to order the clusters in the visualizations.
...
`r Biocpkg("seqArchR")` (for clusters), they can be seamlessly used to utilize the full scope 
of `r Biocpkg("seqArchRplus")`.
...
promoter sequences. 
These were then processed with `r Biocpkg("seqArchR")` to cluster the promoter DNA sequences.
...
If you are not using the result from `r Biocpkg("seqArchR")` or are already using a curated 
set of clusters, you can skip the following subsection and jump to section 
\@ref(generating-plots) for a demonstration of generating all plots.

### Curating raw clusters from `r Biocpkg("seqArchR")` result {#curate-seqArchR-result}

Raw clusters from `r Biocpkg("seqArchR")` result (say, the final iteration) often require 
curation. 

The `r Biocpkg("seqArchR")` result does have a final `clustSol` (the clustering solution) 
...
The motive of Figure \@ref(fig:curation-call-1) is to look at the clusters ordered by 
...
The following subsections demonstrate how the `r Biocpkg("seqArchRplus")` functions can be 
...
##
iqw_tpm_pl <- iqw_tpm_plots(sname = "RAL28_10_12",
                            dir_path = NULL,
                            info_df = info_df,
...
function `plot_motif_heatmaps` (based on Biocpkg("heatmaps"))) or 
`plot_motif_heatmaps2` (based on Biocpkg("seqPattern").
...
or is intergenic.
`r Biocpkg("seqArchRplus")` enables visualizing these annotation proportions per cluster as 
...
The `r Biocpkg("seqArchRplus")` function `per_cluster_strand_dist` can enable visualizing as 
...
```{r go-enrichment, eval=TRUE, attr.source='.numberLines', fig.height=15, fig.width=15, fig.wide=TRUE}
...
### Large panels combining together multiple plots

```{r large-panels, eval=TRUE, attr.source='.numberLines', fig.wide=TRUE, fig.height=8, fig.width=20, fig.cap="Multiple plots combined as panels to geenrate one large figure."}
## Remove y-axis text/tick labels
annot_new_pl <- annotations_oneplot_pl + ggplot2::ylab(NULL) + 
                        ggplot2::theme(axis.text.y = element_blank())

panel_pl <- cowplot::plot_grid(iqw_tpm_pl, seqlogos_oneplot_pl, 
                            annot_new_pl, ncol = 3, axis = "tblh", 
                            align = "h")
panel_pl
```
...
sampleas to produce a list of such plots. 
...
The relevant `r Biocpkg("seqArchRplus")` function that writes to disk this information 
...
for all functions.
Looking at their annotations, if some clusters happen to 
...
You can let `r Biocpkg("seqArchRplus")` generate all plots using default settings like so.
...
experiment.
Clusters of promoter sequences (identified by `r Biocpkg("seqArchR")` or otherwise) can be 
further analyzed with `r Biocpkg("seqArchRplus")`.
